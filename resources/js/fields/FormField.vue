    ...
    <template #field>
      <div class="nova-file-manager">
        <div :class="darkMode && 'dark'">
          <div class="flex flex-row gap-2 flex-wrap w-full" v-if="selection.length > 0">
            <draggable
                v-model="files"
                @start="drag=true"
                @end="drag=false"
            ...
            >
              <template #item="{element}">
                <FieldCard :file="element" mode="form" class="cursor-grab"/>
              </template>
            </draggable>
          </div>
...
export default {
  ...
  methods: {
    ...
    ...mapMutations('nova-file-manager', ['init', 'destroy', 'setSelectedFile', 'setIsFieldMode', 'setValue', 'setLimit', 'deselectFile', 'setSelection']),
    fill(formData) {
      formData.append(
