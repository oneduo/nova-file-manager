<template>
  ...
  <UploadCropModal v-if="uploadIsOpen" :file="file" name="upload-crop" :on-submit="submitCrop" :data="cropData"/>
</template>
...
const uploadIsOpen = computed(() => store.getters['nova-file-manager/allModals'].includes('upload-crop'))
...
const submitCrop = (name) => {
  ...
  store.dispatch('nova-file-manager/uploadCrop', file)
    .then((result) => {
      console.log(result)
